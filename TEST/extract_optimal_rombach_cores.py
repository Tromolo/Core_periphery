...
all_core_data = []
print(f"\nSpúšťam extrakciu core uzlov pre Rombach algoritmus (optimálne parametre)...")

try:
    rombach_algorithm = get_algorithm_function("Rombach")
except ValueError as e:
    print(f"Chyba: Nepodarilo sa získať Rombach algoritmus: {e}")
    sys.exit(1)

for network_name, params in rombach_optimal_params.items():
    ...
    print(f"  Spracúvam {network_name} s alpha={alpha}, beta={beta}, num_runs={num_runs}...")
    
    start_time = time.time()
    
    seed = FIXED_SEED + int(alpha * 100) + int(beta * 1000) + num_runs
    random.seed(seed)
    np.random.seed(seed)
    ...
    try:
        ...
        print(f"  CHYBA pri spustení Rombach algoritmu pre {network_name} (params={params}): {e}")
        traceback.print_exc()
...
if all_core_data:
    ...
    try:
        ...
        print(f"\nZoznamy core uzlov pre Rombach uložené do: {output_csv}")
    except Exception as e:
        print(f"\nCHYBA pri ukladaní výsledkov do {output_csv}: {e}")
else:
    print("\nNeboli vygenerované žiadne dáta o core uzloch.")

print("\nSkript extract_optimal_rombach_cores.py dokončený.")